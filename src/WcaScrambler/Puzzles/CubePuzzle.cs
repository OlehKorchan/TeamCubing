using WcaScrambler.Solvers;
using WcaScrambler.Utils;
using static WcaScrambler.Utils.Assertion;
...
public class CubePuzzle : Puzzle
{
    ...
    private static readonly Dictionary<Face, string> FaceRotationsByName = new()
    {
        [Face.R] = "x",
        [Face.U] = "y",
        [Face.F] = "z",
    ...
    protected CubeMove[][] GetRandomOrientationMoves(int thickness)
    {
        ...
        var randomOrientationMoves =
            new CubeMove[randomUFaceMoves.Length * randomFFaceMoves.Length][];
        var i = 0;
    ...
    public class CubeMove
    {
        ...
        public CubeMove(Face face, int dir, int innerSlice, CubePuzzle p) : this(
            face,
            dir,
            innerSlice,
            0,
            ...
            switch (InnerSlice)
            {
                ...
                default:
                    if (InnerSlice == _puzzle.Size - 1)
                    {
                        ...
                        if (!FaceRotationsByName.ContainsKey(Face))
                        {
                            return null;
                        }
    ...
    public class CubeState : PuzzleState
    {
        ...
        {
            _puzzle = cp;
            Image = ArrayExtension.New<int>(6, _puzzle.Size, _puzzle.Size);

            for (var face = 0; face < Image.Length; face++)
        ...
        {
            return _puzzle.IsNormalized(Image);
        }
        ...
        public override PuzzleState GetNormalized()
        {
            ...
            var normalizedImage = _puzzle.Normalize(Image);
        ...
        public TwoByTwoState ToTwoByTwoState()
        {
            ...
            for (var i = 0; i < pieces.Length; i++)
            {
                ...
                var pieceVal = colorToVal[stickers[0]] +
                               colorToVal[stickers[1]] +
                               colorToVal[stickers[2]];
        ...
        public string ToFaceCube()
        {
            ...
            return faces.Select(f => Image[(int)f])
                .Aggregate(
                    "",
                    (current2, faceArr) =>
                        faceArr.Aggregate(
        ...
        private LinkedHashMap<string, PuzzleState> GetSuccessorsWithinSlice(
            int maxSlice,
            bool includeRedundant)
        {
            ...
            for (var innerSlice = 0; innerSlice <= maxSlice; innerSlice++)
            {
                ...
                {
                    var halfOfEvenCube =
                        _puzzle.Size % 2 == 0 && innerSlice == _puzzle.Size / 2 - 1;
                    ...
                    for (var dir = 1; dir <= 3; dir++)
                    {
                        ...
                        var imageCopy = CloneImage(Image);
